...
function resultNotice(result: ProjectContentImportResult): string {
  ...
  if (result.imported.length)
    parts.push(
      `Added ${result.imported.length} file${result.imported.length === 1 ? "" : "s"}.`,
    );
...
export function ProjectContentDrawer({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<ProjectContentKind, string>>
  >({});
  const [dragging, setDragging] = useState(false);
  const [notice, setNotice] = useState("");
  const [conflict, setConflict] = useState<ConflictState | null>(null);
  const [deleteEntry, setDeleteEntry] = useState<ProjectContentEntry | null>(
    null,
  );
  ...
  const activeConfig =
    contentKinds.find((entry) => entry.kind === activeKind) ?? contentKinds[0];
  ...
  async function importFiles(files: File[], overwrite = false) {
    ...
    setNotice("");
    try {
      const result = await api.project.content.importFiles(
        projectId,
        activeKind,
        files,
        overwrite,
  ...
  async function confirmDelete() {
    ...
    setWorking(true);
    try {
      await api.project.content.trash(
        projectId,
        deleteEntry.kind,
  ...
  async function toggleEntry(entry: ProjectContentEntry) {
    ...
    setNotice("");
    try {
      await api.project.content.setEnabled(
        projectId,
        entry.kind,
        entry.relativePath,
        nextEnabled,
  ...
  const activeEntries = entries[activeKind].filter((entry) =>
    ...
    <>
      ...
      >
        ...
        >
          {contentKinds.map((item) => (
            ...
            >
              ...
              <small>
                {loading[item.kind] ? "…" : entries[item.kind].length}
              </small>
            </button>
          ))}
        </div>
        ...
        <label className="search-field">
          <span className="sr-only">ค้นหาคอนเทนต์</span>
          <input
        ...
        <div
          ...
          onDrop={handleDrop}
        >
          <span className="content-drop-icon" aria-hidden="true">
            ↓
          </span>
        ...
        <div className="content-list-heading">
          ...
          <span>
            ...
            {activeEntries.length === 1 ? "file" : "files"}
          </span>
        </div>

        <div className="content-entry-list" role="tabpanel" key={activeKind}>
          {loading[activeKind] ? (
            <div className="content-state">Loading files…</div>
          ) : null}
          {!loading[activeKind] && activeError ? (
            <div className="content-state error">
              <span>{activeError}</span>
              <button type="button" onClick={() => void loadKind(activeKind)}>
                Retry
              </button>
            </div>
          ) : null}
          ...
          {!loading[activeKind] && !activeError
            ? activeEntries.map((entry) => (
                <article
                  className={`content-entry ${entry.enabled ? "enabled" : "disabled"}`}
                  key={entry.relativePath}
                >
                  <span className="content-entry-file" aria-hidden="true">
      ...
      {conflict ? (
        <div className="content-modal-backdrop" role="presentation">
          ...
          >
            <span className="content-modal-mark" aria-hidden="true">
              !
            </span>
            <h2 id="content-conflict-title">Replace existing files?</h2>
            <p>
              {conflict.result.conflicts.length} file
              {conflict.result.conflicts.length === 1 ? "" : "s"} already exist
              in this folder.
            </p>
            <div className="content-conflict-names">
              {conflict.result.conflicts.map((name) => (
      ...
      {deleteEntry ? (
        <div className="content-modal-backdrop" role="presentation">
          ...
          >
            ...
            <p>
              <strong>{deleteEntry.name}</strong> will be removed from{" "}
              {projectTitle}. You can restore it from the Recycle Bin.
            </p>
            <div className="content-modal-actions">
              <button
                type="button"
